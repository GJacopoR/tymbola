    ...
    <PageTransition isDirectionBack={true}>
      <section className={classes.callerSection}>
        ...
        <ol className={classes.historyList}>
          {history.map((tombolaNumber) => (
            <li className={classes.historyNumber} key={tombolaNumber?.number}>
              {`${tombolaNumber.number}${
                history.indexOf(tombolaNumber) === history.length - 1 ? "" : ","
              }`}
            </li>
          ))}
        </ol>
        ...
        <main className={classes.bodyContainer}>
          <article className={classes.lastExtractedNumber}>
            {history && history[history.length - 1]?.number}
          </article>

          <main className={classes.lowerBodyContainer}>
            ...
            <article className={classes.callerCardsContainer}>
              {callerCards.map((callerCard, i) => (
                <CallerCard callerCard={callerCard} key={"caller_card_" + i} />
              ))}
            </article>
